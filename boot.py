...
def after_install(options,home_dir):
    subprocess.call([join(home_dir,'bin','pip'),'install','tornado'])
    subprocess.call([join(home_dir,'bin','pip'),'install','tornadomail'])
    subprocess.call([join(home_dir,'bin','pip'),'install','python-magic'])
    subprocess.call([join(home_dir,'bin','pip'),'install','beautifulsoup4'])
    subprocess.call([join(home_dir,'bin','pip'),'install','markdown2'])
    subprocess.call([join(home_dir,'bin','pip'),'install','xmltodict'])
    subprocess.call([join(home_dir,'bin','pip'),'install',
        'https://fedorahosted.org/releases/s/u/suds/python-suds-0.4.tar.gz'])
    
    conf = getDBconf()
        
    createStarter(home_dir)
    createSQL(conf['host'])
    ...
    try:
        ...
        print "python-mysqld installed UNSUCCESSFULLY. \n\nPlease install it manually."
    
    subprocess.call('mysql -h %s -u %s -p%s < sql/entu.sql'%(conf['host'],conf['user'],conf['pass']),shell = True)

def createStarter(env_address):
    output = open('starter.py','w')
    output.write(STARTER_FILE%env_address)
    output.close()
